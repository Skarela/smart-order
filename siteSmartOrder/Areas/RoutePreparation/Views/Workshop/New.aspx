...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div class="row-fluid" style="margin-bottom: 30px;">
        <div class="span8 offset2">
            ...
            <h3>Nueva Revisión de Taller</h3>
            <% Html.RenderPartial("_Alerts"); %>
            
            <% using (Html.BeginForm("Create", "Workshop", FormMethod.Post, new { id = "Form", enctype = "multipart/form-data" }))
               { %>
                <div class="tab-pane active">
                    <% Html.RenderPartial("_Form", Model); %>
                    <%: Html.Action("_Form", "Survey", new{surveyId=Model.SurveyId}) %>
                </div>
                <%= Html.ButtonsForm(Url.Action("Index", "Workshop", new {Area = "RoutePreparation"}), true) %>
            <% } %>
        </div>
    </div>
